...
public:
    ...
    static void* MyMalloc(size_t pSize, int pBlock, const char* pFile, int pLine)
    {
        char *p = (char*)_malloc_dbg(pSize+1, pBlock, pFile, pLine);
        *p = '#';
        return p+1;
    }

    static void* MyCalloc(size_t pCount, size_t pSize, int pBlock, const char* pFile, int pLine)
    {
        char *p = (char*)_calloc_dbg(pCount, pSize+1, pBlock, pFile, pLine);
        *p = '#';
        return p+1;
    }

	static void* MyRealloc(void* pData, size_t pSize, int pBlock, const char* pFile, int pLine)
    {
        if (pData)
        {
            ...
            if (*((char*)pData-1)=='#')
            {
                char *p = (char*)_realloc_dbg((char*)pData-1, pSize+1, pBlock, pFile, pLine);
                *p = '#';
                return p+1;
            }
            else
            {   // Mismatch
                char *p = (char*)_realloc_dbg((char*)pData, pSize+1, pBlock, pFile, pLine);
                *p = '#';
                return p+1;
            }
        }
        ...
        {
            char *p = (char*)_realloc_dbg(NULL, pSize+1, pBlock, pFile, pLine);
            *p = '#';
            return p+1;
        }
    }

	static void MyFree(void* pData, int pBlock)
    {
        if (pData==NULL)
            return;
        FBX_ASSERT(*((char*)pData-1)=='#');
        if (*((char*)pData-1)=='#')
        {
            _free_dbg((char*)pData-1, pBlock);
        }
        else
        {   // Mismatch
            _free_dbg(pData, pBlock);
        }
    }

    static size_t MyMsize(void* pData, int pBlock)
    {
        FBX_ASSERT(*((char*)pData-1)=='#');
        if (*((char*)pData-1)=='#')
        {
            return _msize_dbg((char*)pData-1, pBlock);
        }
        else
        {   // Mismatch
            return _msize_dbg((char*)pData, pBlock);
        }
    }
#else
...
#endif /* defined(_DEBUG) && defined(FBXSDK_ENV_WIN) */
};
...
int main(int argc, char** argv)
{
    ...
    FbxSetMSizeHandler(MyMemoryAllocator::MyMsize);
    ...
    gSceneContext = new SceneContext(lFileName, DEFAULT_WINDOW_WIDTH, DEFAULT_WINDOW_HEIGHT, lSupportVBO);
