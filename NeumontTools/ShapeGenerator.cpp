...
#define Padding 0, glm::vec3(0.0f,0.0f,0.0f), glm::vec3(0,0,0), glm::vec3(0,0,0)
namespace Neumont
{
	void ShapeGenerator::findMinMaxPoints(ShapeData& shapeData)
	{
	...
	void ShapeGenerator::calculateTangents(Neumont::ShapeData& shapeData)
	{
		// see http://www.opengl-tutorial.org/intermediate-tutorials/tutorial-13-normal-mapping/ for reference
		//grab each triangle to calculate the tangents from
		for(uint i=0;i<shapeData.numIndices && shapeData.numIndices >= 3;i+=3)
		{
			//short cuts for verts
			...
			// grab the uv vectors from the starting vertex (vert1)
			glm::vec2 deltaUV1 = uv1-uv0;
			glm::vec2 deltaUV2 = uv2-uv0;

			//we want to create the tangent and bitangent parrallel
			//to the uv's
			//we also want to allow larger triangles to have larger tangets/bitangents
			// (larger triangles have more affect on the result)

			//1 over the cross product of our uv's to scale our tangents/bitangents
			float r = 1.0f / (deltaUV1.x * deltaUV2.y - deltaUV1.y * deltaUV2.x);
			//we find the tangent by solving for the difference in uv y directions
			glm::vec3 tangent = (deltaPos1 * deltaUV2.y   - deltaPos2 * deltaUV1.y)*r;
			//we find the tangent by solving for the difference in uv x directions
			glm::vec3 bitangent = (deltaPos2 * deltaUV1.x   - deltaPos1 * deltaUV2.x)*r;

			//check if the tangent/bitangent are right handed 
			//if they are currently left handed negate the tangent
			...
			//save them away in all 3 verts for the triangle
	...
	ShapeData ShapeGenerator::makeLine()
	{
		...
		ret.indices[0] = 0;
		ret.indices[1] = 1;
		findMinMaxPoints(ret);
		calculateTangents(ret);
		return ret;
	}
	...
	ShapeData ShapeGenerator::makePlaneVerts(uint width, uint height)
	{
		ShapeData ret;
		ret.numVerts = width * height;
		ret.verts = new Neumont::Vertex[ret.numVerts];
		for(int i = 0; i < width; i++)
		{
			for(int j = 0; j < height; j++)
			{
				Neumont::Vertex& thisVert = ret.verts[i * height + j];
				thisVert.position.x = j - height * 0.5;
				thisVert.position.z = i - width * 0.5;
				thisVert.position.y = 0;
				thisVert.color = randomColor();
				thisVert.normal = glm::vec3(0, 1, 0);
				thisVert.uv.x = j / (float)(height - 1);
				thisVert.uv.y = i / (float)(width - 1);
				thisVert.boneID =0;
	...
	ShapeData ShapeGenerator::makePlaneIndices(uint width, uint height)
	{
		ShapeData ret;
		ret.numIndices = (width - 1) * (height - 1) * 2 * 3; // 2 triangles per square, 3 indices per triangle
		...
		for(int row = 0; row < width - 1; row++)
		{
			...
			{
				ret.indices[runner++] = height * row + col;
				ret.indices[runner++] = height * row + col + 1;
				ret.indices[runner++] = height * row + col + height + 1;

				ret.indices[runner++] = height * row + col + height + 1;
				ret.indices[runner++] = height * row + col + height;
				ret.indices[runner++] = height * row + col;
			}
		}
	...
	ShapeData ShapeGenerator::makePlane(uint width, uint height)
	{
		Neumont::ShapeData ret = makePlaneVerts(width,height);
		Neumont::ShapeData ret2 = makePlaneIndices(width,height);
	...
	ShapeData ShapeGenerator::makeCube()
	{
		...
		Neumont::Vertex stackVerts[] = 
		{
			...
			vec4(+0.0f, +1.0f, +1.0f, +1.0f), // Color
			vec3(-1.0f, +0.0f, +0.0f),        // Normal
			vec2(+0.0f, +1.0f),               // UV
			Padding,
			...
			vec4(+1.0f, +0.3f, +1.0f, +1.0f), // Color
			vec3(+0.0f, -1.0f, +0.0f),        // Normal
			vec2(+0.0f, +1.0f),               // UV
			Padding,
	...
	{
		ShapeData ret = makePlaneVerts(tesselation,tesselation);
		ShapeData ret2 = makePlaneIndices(tesselation,tesselation);
		ret.indices = ret2.indices;
		ret.numIndices = ret2.numIndices;
